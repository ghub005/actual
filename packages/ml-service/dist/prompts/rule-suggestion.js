export function buildRuleSuggestionPrompt(uncategorized, existingPatterns, categories) {
...
export function parseRuleSuggestionResponse(response) {
    try {
        const cleaned = response.replace(/```json\n?/g, '').replace(/```\n?/g, '').trim();
        const parsed = JSON.parse(cleaned);
        if (!Array.isArray(parsed))
            return [];
        return parsed.filter(r => r.pattern?.field &&
            r.pattern?.operator &&
            r.pattern?.value &&
            r.action?.categoryId);
    }
...
//# sourceMappingURL=rule-suggestion.js.map